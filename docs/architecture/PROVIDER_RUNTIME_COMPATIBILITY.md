...
- ACP model catalogs are provider-session scoped unless an explicit profile
  ...
  control set. Grok's versioned initialize metadata is the current exception,
  and every selected Grok model is revalidated by the target session before a
  prompt;
...
- OpenCode permission rules do not provide an OS sandbox. Both its approval-
  ...
  protects server access; it does not confine that process.
