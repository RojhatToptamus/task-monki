...
Only current-schema task-owned attachment records are accepted. Older
content-addressed records and `storageKey` metadata are unsupported and are not
migrated or reinterpreted. An unsupported store must be discarded before Task
Monki starts with a fresh current-schema store.
