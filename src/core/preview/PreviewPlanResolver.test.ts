...
describe('PreviewPlanResolver', () => {
  ...
  it('folds normalized Compose inspection and exact engine authority into approval', async () => {
    ...
    const plan = await resolver.resolve({
      ...
      worktree: { ownership: 'MANAGED', id: 'worktree', taskId: 'task', iterationId: 'iteration', repositoryId: 'repository-1', worktreePath, branchName: 'codex/task', baseSha: 'base', status: 'PRESENT', createdAt: now, updatedAt: now },
